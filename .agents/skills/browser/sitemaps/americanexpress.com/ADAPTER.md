---
name: american-express
site: americanexpress.com
driver: Browser Use CLI 3.0
profile: adithya
---

# American Express Account Portal Browser Adapter

## Purpose
American Express credit card management, statement balances, transactions, and Membership Rewards.

## Automation Standard
Automated through Browser Use CLI 3.0 via dedicated Chrome CDP port isolation:
```bash
./.agents/skills/browser/cli/browser opencli --site americanexpress.com --profile adithya --url https://www.americanexpress.com/en-us/account/login
```

## Key Capabilities & Workflows
- **Navigation**: Direct deep-linking via sitemap routes (`SITE.md`).
- **Inspection**: Semantic element anchors and accessibility roles (`sitemap.json` and `pages/`).
- **Profile Persistence**: Stateful Chrome session stored under `/home/shakstzy/HADES/.agents/skills/browser/sitemaps/americanexpress.com/profiles/adithya/user_data`.

## Boundaries & Human Intervention
- Stop immediately on CAPTCHA, SMS/Email OTP checkpoints, or biometric / passkey prompts.
- Never log, export, or leak Bitwarden secrets or session cookies into chat or transcripts.
- Stop on card verification codes or two-factor SMS prompts. Never authorize payments or credit line changes.
