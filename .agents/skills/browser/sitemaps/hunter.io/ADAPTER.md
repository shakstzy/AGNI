...
# Hunter Browser Adapter

## Purpose
Hunter.io professional email lookup, domain search, email verification, and API key management.

## Automation Standard
Automated through Browser Use CLI 3.0 via dedicated Chrome CDP port isolation:
```bash
./.agents/skills/browser/cli/browser opencli --site hunter.io --profile adithya --url https://hunter.io/api-keys
```
...
- **Profile Persistence**: Stateful Chrome session stored under `/home/shakstzy/HADES/.agents/skills/browser/sitemaps/hunter.io/profiles/adithya/user_data`.
